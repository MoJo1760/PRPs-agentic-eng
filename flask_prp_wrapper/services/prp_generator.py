...
class PRPGenerator:
    ...
    def __init__(
        self, prp_templates_path: str = "PRPs/templates", min_quality_score: float = 8.0
    ):
    ...
    async def generate_prp(
        ...
        if not template:
            ...
            template = self._templates_cache.get(
                "prp_base", self._get_fallback_template()
            )
        ...
        if quality_metrics["overall_score"] < self.min_quality_score:
            logger.info(
                f"PRP quality {quality_metrics['overall_score']:.1f} below threshold, enhancing..."
            )
        ...
        complexity = self._estimate_complexity(
            concept, questionnaire_session, research_context
        )
    ...
    async def _populate_prp_template(
        ...
        questionnaire_summary = self._summarize_questionnaire_responses(
        ...
        sections = {
            ...
            "SUCCESS_DEFINITION": self._generate_success_definition(
                concept, questionnaire_summary
            ),
            ...
            "IMPLEMENTATION_TASKS": self._generate_implementation_tasks(
                concept, research_context, questionnaire_summary
            ),
            "VALIDATION_LOOP": self._generate_validation_loop(
                concept, research_context
            ),
            "ANTI_PATTERNS": self._generate_anti_patterns(
                concept.domain.value if concept.domain else "general", research_context
            ),
        }
        ...
        populated_template = populated_template.replace(
            "[BUSINESS_DOMAIN]", concept.domain.value if concept.domain else "general"
        )
        populated_template = populated_template.replace(
            "[BUSINESS_MODEL]",
            concept.business_model.value if concept.business_model else "not specified",
        )

        return populated_template

    def _summarize_questionnaire_responses(
    ...
    def _generate_feature_goal(
        self, concept: BusinessConceptRequest, questionnaire_summary: Dict
    ) -> str:
    ...
    def _generate_success_definition(
        ...
        return (
            f"The {concept.title} application is complete when:\n"
    ...
    def _generate_target_user(
        ...
        return domain_users.get(
            concept.domain.value if concept.domain else "general",
            "End users seeking digital solutions",
        )
    ...
    def _generate_pain_points(
        ...
        if concept.domain:
            domain_pains = {
                ...
                "mobile_app": [
                    "Limited mobile access to services",
    ...
    def _generate_why_section(
        self, concept: BusinessConceptRequest, research_context: ResearchContext
    ) -> str:
        ...
        if research_context.competitor_analysis:
            why_points.append(
                f"**Competitive Advantage**: Improvements over existing solutions like {', '.join(research_context.competitor_analysis[:2])}"
            )

        return "\n".join(why_points)

    def _generate_what_section(
        self, concept: BusinessConceptRequest, questionnaire_summary: Dict
    ) -> str:
        ...
        if concept.domain:
            domain_criteria = {
                "saas": [
                    ...
                    "Subscription billing processes payments",
                ],
                "e_commerce": [
                ...
                "mobile_app": [
                    "Mobile API responses are optimized",
    ...
    def _generate_anti_patterns(
        self, domain: str, research_context: ResearchContext
    ) -> str:
    ...
    def _assess_prp_quality(
        self, prp_content: str, research_context: ResearchContext
    ) -> Dict[str, float]:
        ...
        present_sections = sum(
            1 for section in required_sections if section.lower() in prp_content.lower()
        )
        metrics["content_completeness"] = (
            present_sections / len(required_sections)
        ) * 10

        # Context richness (0-10)
        context_indicators = len(research_context.official_documentation_links) + len(
            research_context.technical_patterns
        )
        ...
        validation_count = sum(
            prp_content.lower().count(keyword) for keyword in validation_keywords
        )
        ...
        technical_count = sum(
            prp_content.lower().count(keyword) for keyword in technical_keywords
        )
    ...
    async def _enhance_prp_content(
        ...
        if quality_metrics["validation_coverage"] < 7.0:
            ...
            if concept.domain:
                ...
                domain_commands = domain_validations.get(
                    concept.domain.value, ["# Test core business logic"]
                )
        ...
        for block in bash_blocks:
            ...
            for line in lines:
                ...
                if line and not line.startswith("#") and not line.startswith("sleep"):
                    ...
                    if any(
                        cmd in line
                        for cmd in ["pytest", "ruff", "mypy", "curl", "python"]
                    ):
    ...
    def _estimate_complexity(
        ...
        complexity_score += domain_complexity.get(
            concept.domain.value if concept.domain else "other", 1
        )
