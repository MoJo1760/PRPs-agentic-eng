...
class PRPIntegrationService:
    ...
    def __init__(self, prps_root_path: str = ".", prp_runner_script: str = "PRPs/scripts/prp_runner.py"):
    ...
    def _validate_environment(self):
        ...
        try:
            ...
            if result.returncode != 0:
                logger.warning("UV package manager not available - PRP execution may fail")
    ...
    async def execute_prp(
        ...
        try:
            ...
            # Build command
            cmd = [
                'uv', 'run', str(self.prp_runner),
                '--prp-path', str(prp_file_path)
            ]
            ...
            try:
                ...
                generated_files = self._extract_generated_files(result.stdout, result.stderr)
    ...
    async def validate_prp_commands(
        ...
        for i, command in enumerate(validation_commands):
            ...
            try:
                ...
                logger.info(f"Validation command completed: {command} (success: {validation_result.success})")
    ...
    async def create_prp_summary(
        ...
        return {
            ...
            'recommendations': self._generate_recommendations(prp_execution_result, validation_results)
        }
    ...
    def _generate_recommendations(
        ...
        if not execution_result.success:
            if "timeout" in execution_result.error_output.lower():
                recommendations.append("Consider breaking down complex PRPs into smaller, more focused tasks")
            elif "permission" in execution_result.error_output.lower():
                recommendations.append("Check file permissions and ensure proper access to project directories")
            elif "command not found" in execution_result.error_output.lower():
                recommendations.append("Verify that all required tools (uv, claude) are installed and accessible")
        
        # Validation-based recommendations
        failed_validations = [r for r in validation_results if not r.success]
        
        if failed_validations:
            syntax_failures = [r for r in failed_validations if "ruff" in r.command or "mypy" in r.command]
            if syntax_failures:
                recommendations.append("Address syntax and type checking issues before proceeding")
            
            test_failures = [r for r in failed_validations if "pytest" in r.command or "test" in r.command]
            if test_failures:
                recommendations.append("Review and fix failing tests - they indicate implementation issues")
            
            if len(failed_validations) > len(validation_results) / 2:
                recommendations.append("Consider revising the PRP with more detailed implementation guidance")
        
        # Performance recommendations
        if execution_result.execution_time_seconds > 600:  # 10 minutes
            recommendations.append("Long execution time - consider optimizing the implementation approach")
        
        # Success recommendations
        if execution_result.success and len(failed_validations) == 0:
            recommendations.append("Excellent! All validations passed - ready for production deployment")
        elif execution_result.success and len(failed_validations) < 3:
            recommendations.append("Good progress - address remaining validation issues for production readiness")
    ...
    async def validate_prp_content(self, prp_content: str) -> Dict[str, Any]:
        ...
        validation_result['missing_sections'] = [s for s in required_sections if s not in sections_found]
        
        # Calculate completeness score
        validation_result['completeness_score'] = len(sections_found) / len(required_sections)
        
        # Check for critical content
        if len(prp_content) < 1000:
            validation_result['warnings'].append("PRP content seems quite short - consider adding more detail")
        
        if 'validation loop' not in content_lower:
            validation_result['warnings'].append("No validation loop section found - validation commands may be missing")
        
        if len(validation_result['missing_sections']) > 2:
            validation_result['errors'].append(f"Missing critical sections: {', '.join(validation_result['missing_sections'])}")
        ...
        if not bash_blocks:
            validation_result['warnings'].append("No bash code blocks found - validation commands may be missing")
