...
class Question(BaseModel):
    ...
    priority: int = Field(default=5, ge=1, le=10, description="Question priority (1=highest)")
    ...
    def add_response(self, response: QuestionnaireResponse):
        ...
        if self.questions:
            ...
            if required_questions:
                self.completion_percentage = (len(answered_required) / len(required_questions)) * 100
            else:
...
class QuestionnaireGenerationRequest(BaseModel):
    ...
    existing_responses: Optional[List[QuestionnaireResponse]] = Field(
        None,
        description="Previous responses to consider"
    )