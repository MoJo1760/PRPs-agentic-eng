...
class BusinessConceptRequest(BaseModel):
    ...
    target_users: Optional[str] = Field(
        None,
        max_length=500,
        description="Who are the target users or customers"
    )
    ...
    existing_competitors: Optional[str] = Field(
        None,
        max_length=1000,
        description="Known competitors or similar solutions"
    )
...
class BusinessConceptResponse(BaseModel):
    ...
    estimated_questions: int = Field(
        ...
        ge=0,
        le=15,
        description="Estimated number of follow-up questions"
    )
...
class ConceptAnalysis(BaseModel):
    ...
    def overall_score(self) -> float:
        ...
        return (self.complexity_score + self.feasibility_score + self.market_potential_score) / 3